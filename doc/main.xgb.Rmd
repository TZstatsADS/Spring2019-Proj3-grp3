...
setwd("/Users/seungwookhan/Documents/Columbia/Senior/Applied Data Science/Spring2019-Proj3-spring2019-proj3-grp3/")
...
```{r model_setup}
model_values <- c(0.3,0.4,0.5,0.6)
#model_values <- c("shotgun", "coord_descent")
#model_values <- c(3,5,7,9,11)
model_labels = paste("XGB with model eta =", model_values)
```
...
source("../lib/XGB/cross_validation.xgb.R")
if(run.cv){
  err_cv_xgboost <- array(dim=c(length(model_values), 2))
  for(k in 1:length(model_values)){
    cat("Running cross validation with updater: ", model_values[k], "\n")
    err_cv_xgboost[k,] <- cv.xgboost(feat_train, label_train, model_values[k], K)
  }
...
```{r cv_vis}
if(run.cv){
  #load("../output/XGB/err_cv_xgboost.RData")
  plot(model_values, err_cv_xgboost[,1], xlab="ETA (learning rate)", ylab="CV Error",
...
if(run.cv){
  ...
  cat("Best model is with depth: ", model_best, "\n")
}
err_cv_xgboost[,1]
...
tm_train=NA
par_best <- NULL
tm_train <- system.time(fit_train_xgboost <- train_xgboost(feat_train, label_train, par_best))
