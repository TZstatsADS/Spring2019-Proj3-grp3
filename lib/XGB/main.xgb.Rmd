...
source("../lib/XGB/cross_validation.xgboost.R")
if(run.cv){
  err_cv_xgboost <- array(dim=c(length(model_values), 2))
  for(k in 1:length(model_values)){
    cat("k=", k, "\n")
    err_cv_xgboost[k,] <- cv.function_xgboost(feat_train, label_train, model_values[k], K)
  }
  save(err_cv_xgboost, file="../output/XGB/err_cv_xgboost.RData")
}
```
...
tm_train=NA
par_best <- NULL
tm_train <- system.time(fit_train_xgboost_3_3 <- train_xgboost(feat_train, label_train, par_best))
save(fit_train_xgboost_3_3, file="../output/XGB/fit_train_xgboost.RData")
```
...
test_dir <- "../data/test_set/XGB/" # This will be modified for different data sets.
test_LR_dir <- paste(test_dir, "LR/", sep="")
test_HR_dir <- paste(test_dir, "HR/", sep="")
tm_test=NA
run.test <- T
if(run.test){
  load(file="../output/XGB/fit_train_xgboost.RData")
