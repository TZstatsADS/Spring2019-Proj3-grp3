...
```{r model_setup}
model_values <- c(3,5,6,8)
model_labels = paste("XGB with model depth =", model_values)
```
...
if(run.cv){
  ...
  for(k in 1:length(model_values)){
    cat("Running cross validation with depth: ", model_values[k], "\n")
...
if(run.test){
  ...
  tm_test <- system.time(superResolution(test_LR_dir, test_HR_dir, fit_train_xgboost))
}
```
